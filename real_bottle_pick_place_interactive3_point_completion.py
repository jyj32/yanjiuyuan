...
def delete_collision_gids(candidate_indices_0, grasp_collection, goal_pose_list, gripper, surrounding_pcd, show=False):
    ...
    for goal_id, goal_pose in enumerate(goal_pose_list):
        ...
        for grasp_index in candidate_indices_0:
            ...
            # 点云转凸包网格 → 临时文件 → CollisionModel
            hull_mesh, _ = collision_pcd.compute_convex_hull()
            temp_path = tempfile.NamedTemporaryFile(suffix=".obj", delete=False)
            temp_path.close()
            o3d.io.write_triangle_mesh(temp_path.name, hull_mesh)
            pcd_cmodel = mcm.CollisionModel(temp_path.name)
            ...
            if gripper.is_mesh_collided(cmodel_list=[pcd_cmodel]):
                ...
                if show:
                    ...
                    # 球点云
                    # for ball_pos in [ball1_pos, ball2_pos]:
                    #     sphere = o3d.geometry.TriangleMesh.create_sphere(radius=r)
                    #     sphere.translate(ball_pos)
            ...
            # 删除临时文件
            Path(temp_path.name).unlink(missing_ok=True)
...
def gen_pick_only_path(robot, object_model_path: Path, grasps, pick_pose, obstacle_list, args: SimpleNamespace) -> tuple[Optional[int], Any]:
    ...
    if sim_pick.USE_REASONED_COMMON_GRASPS:
        grasp_collection = sim_pick.make_grasp_collection(robot, grasps)    # 把 grasp 列表包装成 WRS 框架认的格式
        # 筛选手爪位姿序列号，（筛选条件：ik有解，机器人和手爪都无碰撞）
        candidate_indices = list(
            planner.reason_common_gids(
                grasp_collection=grasp_collection,  # 抓取位姿集合
                goal_pose_list=[pick_pose], # 目标瓶子的位姿
                obstacle_list=obstacle_list,    # 障碍物
                toggle_dbg=False,
            )
        )
        # 通过周围点云进行筛选
    ...
    push_settings = action_sequence_push_settings(args) # 是否推还是正常抓取
    if push_settings is not None:   # 推
    ...
    for grasp_index in candidate_indices:
        ...
        if push_settings is not None:   # 推开
...
def run_or_skip_plan(args: SimpleNamespace, icp: ObjectIcpResult) -> Optional[PlanningResult]:
    ...
    if selected_grasp_index is None:    # 规划成功但没返回 grasp 序号时，从轨迹数据反推用的是哪个 grasp
    ...
    # 把规划阶段的所有结果打包成一个数据类，方便后续执行阶段统一取用
