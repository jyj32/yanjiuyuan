...
REAL_PIPELINE_CONFIG = {
    ...
    # ---- YOLO 瓶子检测参数 ----
    "yolo_model": Path(__file__).resolve().parent / "models" / "bottle_detect2.pt",
    "yolo_conf": 0.528,
    "yolo_iou": 0.70,
    "no_yolo": False,
    ...
    "action_sequence": 1,
    "grasp_pickle": DEFAULT_GRASP_PICKLE_PATH,
    "dry_run": False,
    "use_rrt": True,
    "no_env": False,
    "box_obstacle": True,
...
def capture_synced_context(args: SimpleNamespace) -> tuple[box_object_icp.PipelineContext, dict]:
    ...
    try:
        ...
        provider.suspend()
        # 相机拍照
        with timed_step("Mech capture"):    # 1.3733s
        ...
        # 箱子位姿检测
        with timed_step("Detect Box"):  # 0.16s
...
def gen_pick_only_path(robot, object_model_path: Path, grasps, pick_pose, obstacle_list, args: SimpleNamespace) -> tuple[Optional[int], Any]:
    ...
    if sim_pick.USE_REASONED_COMMON_GRASPS:
        ...
        sim_pick.debug_print(f"  filter pick grasps result: {len(candidate_indices)}/{len(grasps)} ({_t_reason:.3f}s)")
    ...
    pick_approach_jaw_width_source = f"default +0.03 grasp jaw"
    push_settings = action_sequence_push_settings(args)
    if push_settings is not None:
    ...
    for grasp_index in candidate_indices:
        ...
        if push_settings is not None:
            ...
            sim_pick.debug_print(
                ...
                f"gen_pick_push_path={_t_path_plan:.3f}s, "
                f"post_pick=0.000s, "
        ...
        sim_pick.debug_print(
            f"[planning_timing] filter_grasps_sequence={_t_reason:.3f}s, "
            f"gen_pick_and_moveto={_t_path_plan:.3f}s, "
