...
WRS key workflow:
  ...
  D: segment the bottle, choose a point-cloud template, run bottle ICP, show start and place poses.
...
REAL_PIPELINE_CONFIG = {
    ...
    "bottle_template_prompt_gui": True,
...
def refresh_sam_task_settings(args: SimpleNamespace) -> Optional[dict[str, Any]]:
    ...
    print(
        ...
        f"{gui_text}; points={point_text}; config={settings['config_path']}",
        flush=True,
    )
    return settings
...
def prepare_interactive_pipeline_context(args: SimpleNamespace) -> box_object_icp.PipelineContext:
...
def decorate_grasp_for_rtde(grasp, grasp_index: int):
    setattr(grasp, "name", f"pickle_grasp_{grasp_index}")
    setattr(grasp, "jaw_width", float(np.asarray(grasp.ee_values, dtype=float).reshape(-1)[0]))
...
def build_and_save_rtde_plan(args: SimpleNamespace, planning: PlanningResult, default_dir: Path) -> tuple[rtde_utils.RtdeExecutionPlan, Path]:
    ...
    grasp = decorate_grasp_for_rtde(grasps[grasp_index], grasp_index)
...
def build_obstacle_lists(args: SimpleNamespace, box_homomat: np.ndarray) -> tuple[list[object], list[object]]:
    ...
    detected_box_panels = make_concave_box_collision_obstacles(box_homomat, show_cdprim=True)
...
def gen_pick_only_path(robot, obj_cmodel, grasps, pick_pose, obstacle_list) -> tuple[Optional[int], Any]:
    ...
    robot.goto_given_conf(sim_pick.DEFAULT_HOME_CONF, ee_values=sim_pick.clamp_jaw_width(robot, sim_pick.OPEN_JAW_WIDTH))
    for grasp_index in candidate_indices:
        ...
        mot_data = planner.gen_pick_and_moveto(
            ...
            pick_approach_jaw_width=sim_pick.clamp_jaw_width(robot, sim_pick.OPEN_JAW_WIDTH),
        ...
        sim_pick.debug_print(f"  Final open gripper frame appended: jaw={final_open_width:.5f} m")

        frame_count = len(mot_data.jv_list) if hasattr(mot_data, "jv_list") else len(mot_data)
        sim_pick.debug_print(
            f"Pick planner produced {frame_count} frame(s), "
            "including unchecked post-pick first-joint alignment."
        )
...
def run_or_skip_plan(args: SimpleNamespace, icp: ObjectIcpResult) -> Optional[PlanningResult]:
    ...
    if sim_pick.VALIDATE_COLLISIONS:
        sim_pick.debug_print("Collision check: passed for pick approach/depart; post-pick J1 alignment is unchecked.")
...
class InteractiveBottlePickPlaceApp:
    def __init__(
        ...
        print("WRS viewer is ready. Press C to sync/capture, D for segmentation/template/ICP, P to plan, O to execute.")
    ...
    def run_sync_capture(self) -> None:
        ...
        try:
            ...
            self.status_text.setText(f"C sync done: {output_dir.name}. Press D to estimate pose.")
    ...
    def run_execute(self) -> None:
        ...
        self.status_text.setText(f"O execution starting ({mode}, {compliant_mode})...")
        rtde_robot = object()
        try:
            if not dry_run:
            ...
            self.status_text.setText(
                f"O execution complete ({mode}, {compliant_mode}): {len(log)} segment(s). Press C before any new D/P/O."
            )
            print(f"[real_pipeline] O execution complete ({mode}, {compliant_mode}); segments={len(log)}")
            for entry in log:
    ...
    def run_plan(self) -> None:
        ...
        try:
            ...
            self.status_text.setText(f"Planning done: {len(planning.mot_data.jv_list)} frames. Press O to execute.")
            print(f"[real_pipeline] summary after planning: {summary_path}{rtde_path}")
        except Exception as exc:
