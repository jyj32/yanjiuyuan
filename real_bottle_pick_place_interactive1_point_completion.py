...
try:
    ...
    Notify.ptr().set_verbosity_synced(4)  # 只显示 fatal 级别
except Exception:
    pass
...
from yanjiuyuan import box_object_pointcloud_from_saved_capture_sam_completion_template_icp as box_object_icp  # noqa: E402
...
BOX_OBJECT_SCRIPT = Path(__file__).resolve().parent / "box_object_pointcloud_from_saved_capture_sam_completion_template_icp.py"
...
REAL_PIPELINE_CONFIG = {
    ...
    "save_capture_pointclouds": True,  # 是否保存点云图
    "rtde_plan_out": None,
...
def capture_synced_context(args: SimpleNamespace) -> tuple[box_object_icp.PipelineContext, dict]:
    ...
    try:
        ...
        provider.suspend()
        with timed_step("capture images"):   # 拍照
...
def pick_approach_jaw_width_for_grasp(robot, grasp) -> float:
    """
        设置实际抓取宽度，比夹紧时宽1.5cm,范围在[0.042, 0.118] m
    """
...
def build_obstacle_lists(
    args: SimpleNamespace,
    box_homomat: np.ndarray,    # 箱子位姿
    include_display: bool = False,  # 展示桌子，箱子
    ...
    if not args.no_env:
        ...
        if include_display:
            display_obstacles.append(table) # 展示桌子
    ...
    if include_display:
        ...
        display_obstacles.extend(detected_box_panels)   # 展示箱子
...
def delete_collision_gids(candidate_indices_0, grasp_collection, goal_pose_list, gripper, surrounding_pcd, show=False):
    """
    删除与周围物体碰撞的手爪位姿序列
    输入:
        candidate_indices_0: 手爪位姿序列候选
        grasp_collection: 抓取总集合
        goal_pose_list: 物体位姿集合
        gripper: 手爪模型
        surrounding_pcd: 物体周围的点云
        show: 是否展示碰撞
    输出:
        candidate_indices_1: 不碰撞的手爪位姿序列候选
    """
    ...
    candidate_indices_1 = list(candidate_indices_0)
    r = 0.05  # 球半径，越大越会碰撞

    # 预构建每个碰撞网格的RaycastingScene（网格几何不变，只需建一次）
    mesh_scenes = []  # [(cdmodel, scene), ...]
    for cdmodel in gripper.cdmesh_list:
        if cdmodel is None or cdmodel._trm_mesh is None:
            continue
        trm = cdmodel._trm_mesh
        verts = np.asarray(trm.vertices)
        faces = np.asarray(trm.faces)
        ...
        mesh_scenes.append((cdmodel, scene))

    for goal_id, goal_pose in enumerate(goal_pose_list):
        goal_pos = goal_pose[0]
        goal_rotmat = goal_pose[1]
        for grasp_index in candidate_indices_0:
            grasp = grasp_collection[grasp_index]
            grasp_width = min(grasp_jaw_width(grasp) + 0.015, 0.118) # 张开时加0.015m,不超过0.118
            goal_jaw_center_pos = goal_pos + goal_rotmat.dot(grasp.ac_pos)
            goal_jaw_center_rotmat = goal_rotmat.dot(grasp.ac_rotmat)

            # 手爪 x 方向（开合方向）
            jaw_x = goal_jaw_center_rotmat[:, 0]
            half_width = grasp_width / 2.0
            ball1_pos = goal_jaw_center_pos + half_width * jaw_x
            ball2_pos = goal_jaw_center_pos - half_width * jaw_x

            # 保留与两球心距离小于 r 的点云
            points = np.asarray(surrounding_pcd.points)
            dist_to_ball1 = np.linalg.norm(points - ball1_pos, axis=1)
            dist_to_ball2 = np.linalg.norm(points - ball2_pos, axis=1)
            mask = (dist_to_ball1 < r) | (dist_to_ball2 < r)
            ...
            collision_pcd.points = o3d.utility.Vector3dVector(points[mask])
            if surrounding_pcd.has_colors():
                colors = np.asarray(surrounding_pcd.colors)
                collision_pcd.colors = o3d.utility.Vector3dVector(colors[mask])

            # 手爪移到 grasp 位姿
            gripper.grip_at_by_pose(
                jaw_center_pos=goal_jaw_center_pos,
                jaw_center_rotmat=goal_jaw_center_rotmat,
                jaw_width=grasp_width,
            )

            # 直接用点进行碰撞检测：用Open3D RaycastingScene检查点是否在手爪碰撞网格内部
            collision_pts = points[mask]  # (N, 3) 世界坐标
            is_collided = False
            for cdmodel, scene in mesh_scenes:
                # 将世界坐标点转换到手爪碰撞网格的局部坐标
                local_pts = (collision_pts - cdmodel.pos) @ cdmodel.rotmat
                # 检查点是否在网格内部
                pts_t = o3d.core.Tensor(np.ascontiguousarray(local_pts, dtype=np.float32))
                occupancy = scene.compute_occupancy(pts_t)
                if np.any(occupancy.numpy() > 0.5):
                    is_collided = True
                    break

            if is_collided:
                candidate_indices_1.remove(grasp_index)
                sim_pick.debug_print(f"  grasp_{grasp_index} collided with surrounding pcd, removed")
                # 可视化碰撞点云：红色=球内点云，灰色=周围点云，绿色=手爪
                if show:
                    vis_list = []
                    # 球内周围物体点云
                    collision_pcd_vis = o3d.geometry.PointCloud()
                    collision_pcd_vis.points = collision_pcd.points
                    collision_pcd_vis.paint_uniform_color([1.0, 0.0, 0.0])
                    vis_list.append(collision_pcd_vis)
                    # 周围物体点云
                    surrounding_vis = o3d.geometry.PointCloud()
                    surrounding_vis.points = surrounding_pcd.points
                    surrounding_vis.paint_uniform_color([0.5, 0.5, 0.5])
                    vis_list.append(surrounding_vis)
                    for cdmodel in gripper.cdmesh_list:
                        if cdmodel is not None and cdmodel._trm_mesh is not None:
                            trm = cdmodel._trm_mesh
                            verts = np.asarray(trm.vertices)
                            verts = (cdmodel.rotmat @ verts.T).T + cdmodel.pos
                            faces = np.asarray(trm.faces)
                            ...
                            gripper_mesh.paint_uniform_color([0.0, 1.0, 0.0])
                            gripper_mesh.compute_vertex_normals()
                            vis_list.append(gripper_mesh)
                    o3d.visualization.draw_geometries(vis_list, window_name=f"grasp_{grasp_index} COLLISION", width=1280, height=720)

    return candidate_indices_1
...
def gen_pick_only_path(robot, object_model_path: Path, grasps, pick_pose, obstacle_list, args: SimpleNamespace) -> tuple[Optional[int], Any]:
    ...
    if sim_pick.USE_REASONED_COMMON_GRASPS:
        with timed_step("choose_grasp_pose"):   # 筛选手爪姿态
            ...
            if remaining_ply_path is None or not Path(remaining_ply_path).exists():
                sim_pick.debug_print("  周围点云不存在, 跳过筛选.")
            else:   # 周围点云存在，可以筛选
                import open3d as o3d
                remaining_pcd = o3d.io.read_point_cloud(remaining_ply_path)
                sim_pick.debug_print(
                    f"  Loaded remaining point cloud: {remaining_ply_path} ({len(remaining_pcd.points)} points)")
                candidate_indices = delete_collision_gids(
                    candidate_indices_0=candidate_indices,
                    grasp_collection=grasp_collection,
                    goal_pose_list=[pick_pose],
                    gripper=robot.end_effector,
                    surrounding_pcd=remaining_pcd,
                    show=False,  # 想看可视化就设 True,但是会报错
                )
                sim_pick.debug_print(f"  周围点云碰撞检测后: {len(candidate_indices)}/{len(grasps)}")
    ...
    for grasp_index in candidate_indices:
        ...
        with timed_step("grasp path plan"):  # 路径规划
...
def run_or_skip_plan(args: SimpleNamespace, icp: ObjectIcpResult) -> Optional[PlanningResult]:
    ...
    # 从 summary JSON 读取 remaining point cloud 路径，传给规划函数
    try:
        summary_json = json.loads(icp.summary_path.read_text(encoding="utf-8"))
        remaining_path = summary_json.get("remaining_pointcloud_path")
        if remaining_path:
            setattr(args, "remaining_pointcloud_path", remaining_path)
            sim_pick.debug_print(f"Remaining point cloud: {remaining_path}")
    except Exception as exc:
        sim_pick.debug_print(f"Warning: could not read remaining_pointcloud_path from summary: {exc}")
    ...
    if sim_pick.VISUALIZE_RESULT:   # 路径规划结果动态展示
...
class InteractiveBottlePickPlaceApp:
    ...
    def attach_detection_result(self, summary: dict, selected_mask: np.ndarray) -> None:
        self.clear_detection_models()   # 清掉旧的障碍物模型
        if self.ctx is not None:
    ...
    def run_detection(self) -> None:
        ...
        self.clear_detection_models()
        try:
            # Force manual click mode: open OpenCV window for the user to click
            # SAM prompt points, same as
            # box_object_pointcloud_from_saved_capture_sam_completion_template_icp.py
            # when run without --point and without --no-gui.
            apply_action_sequence_template_settings(self.args)
            self.args.no_gui = False
            self.args.point = []
            self.args.show_gui_with_points = False
            self.ctx.args.no_gui = False
            self.ctx.args.point = []
            self.ctx.args.show_gui_with_points = False
            print("[real_pipeline] SAM manual click mode: OpenCV window will open for point selection.", flush=True)
