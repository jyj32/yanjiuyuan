...
from yanjiuyuan.yolo_detect import BottleDetector  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    # ---- YOLO 瓶子检测参数 ----
    parser.add_argument("--yolo-model", type=Path, default=Path(__file__).parent / "models" / "bottle_detect.pt", help="YOLO 瓶子检测模型路径")
    parser.add_argument("--yolo-conf", type=float, default=0.7, help="YOLO 置信度阈值")
    parser.add_argument("--yolo-iou", type=float, default=0.5, help="YOLO NMS IoU 阈值")
...
def run_or_load_point_hint_mask(
    ...
    if use_yolo:
        ...
        # yolo检测水平框
        detections = yolo_detector.detect_bottle(image_bgr, "", show=False, save=False)
        if not detections:
        ...
        if use_yolo:
            # 取面积最大的瓶子（detections 已按面积降序排列）
            best = detections[0]
            x1, y1, x2, y2 = best["bbox"]
            log(f"[box_object] YOLO 选用: "
                f"bbox=({x1},{y1})-({x2},{y2}), conf={best['confidence']:.3f}, area={best['area']:.0f}")
...
def run_completion_bottle_icp_on_selected_and_completed(
    ...
    ransac_n = max(3, int(config.global_ransac_n))  # 3次循环

    old_icp_max_iteration = bottle_icp.ICP_MAX_ITERATION
    bottle_icp.ICP_MAX_ITERATION = int(config.icp_max_iteration)
    try:
        # 2.RANSAC粗配准（global registration），3次循环，
        with timed_step("RANSAC粗配准（ICP）"): # 2.79s➡2.64s，
        ...
        # 4.ICP 精配准，
        with timed_step("ICP精配准（ICP）"): # 0.01s
...
def run_segmentation_and_bottle_icp_attempt(ctx: PipelineContext) -> tuple[dict, ExtractionMasks, np.ndarray]:
    ...
    with timed_step("project 2D mask to 3D and select one object"): # 0.01s
        ...
        segmentation_elapsed_s = perf_counter() - segmentation_start    # yolo检测，sam分割+取出点云

    with timed_step("write extraction summary"):    # 0.01s
    ...
    if getattr(attempt_args, "completion_matching", False):
        # 补全完整点云
        with timed_step("complete selected point cloud with AdaPoinTr+ICP"):    # 8.51s➡6.42s➡4.68s，4.39s
            matching_start = perf_counter()
            ...
            with timed_step("complete selected point cloud +RANSAC 粗配准 + ICP 精配准"): # 4.57s➡3.94s➡2.93，3.06s
    ...
    summary["timing"] = {
        "segmentation_elapsed_s": segmentation_elapsed_s,   # yolo检测，Sam分割，取出点云时间
